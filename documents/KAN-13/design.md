...
- 파일맵 — 생성: 컴포넌트 `CP-06-01`~`CP-06-05` 의 파일과 각 테스트 파일 (10장 구현 컴포넌트 목록), `apps/web/src/hooks/use-storage-health.ts`(+`use-storage-health.test.ts` — 탭 셸이 부르던 헬스 조회를 훅으로 추출). 수정: `CP-05-01`(`apps/web/src/App.tsx` — 헬스 조회를 그 훅으로 옮기고 내 쿠폰 탭 연결), `documents/KAN-13/design.md`(결정 12·결정 10 근거 보강·헤딩 레벨 규칙·`t-note` 규칙과 이 브랜치의 커밋 계획 — 4·9~11·13·14장).
