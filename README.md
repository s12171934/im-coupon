...
API 를 띄우지 않고 시드를 넣으려면 저장소 루트에서 실행한다. 필요한 패키지는 자동으로 빌드한다.

```bash
pnpm db:seed
```

기존 컬렉션이 하나라도 있으면 전체 초기화를 건너뛰어 런타임 데이터를 보존한다.
실행 후 DB 경로·스키마 버전·컬렉션별 건수를 출력한다.
API 와 동일하게 `IM_COUPON_DATA_DIR` (대상 DB), `IM_COUPON_SEED_DIR` (시드 원본) 환경변수로 경로를 지정할 수 있다.
