...
이 저장소는 **자기완결**이다. 커밋되는 어떤 파일도 저장소 밖 원본의 존재를 드러내지 않는다.
공모전 제출물이자 팀 공유물이므로 개인 작업 환경이 문서에 새어 나가면 안 된다.
...
- 저장소 밖 파일 경로 — 홈 디렉터리 절대경로, 개인 노트 폴더 구조, 개인 머신 경로
- "밖에 원본이 따로 있다"는 서술 자체. 상류·SSOT·동기화 방향을 저장소 문서에서 설명하지 않는다
- 사내 위키·이슈 트래커 등 팀 밖 시스템의 내부 링크

**대신 이렇게 쓴다**

| 쓰지 않는 표현 | 대체 |
| --- | --- |
| "`<외부 노트 이름>` 이 원본" | 근거를 이 문서에 직접 옮겨 적고 출처 줄을 생략한다 |
| "외부 문서에 결정되어 있음" | 결정 내용을 `- YYYY-MM-DD — <결정>` 으로 여기에 적는다 |
| "`~/...` 의 문서 참고" | 저장소 상대 경로(`documents/<파일>.md`)로만 참조한다 |
| "아직 옮기지 않음, 원본은 밖에 있음" | `- [ ] <주제> — 아직 기록되지 않음` 으로 주제만 남긴다 |

**옮길 때의 판단** — 밖에서 가져오는 것은 *결론과 근거*이지 *출처*가 아니다.
근거가 저장소 문서만으로 서지 않으면 그 근거까지 옮겨 적는다. "밖에 자세히 있다"로 미루지 않는다.
